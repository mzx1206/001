...
# ==================== 生成监控地图 ====================
def make_monitor_map(center, waypoints, obs, hist, full_path, drone_pos, drone_alt, safe_rad):
    """专门用于监控页面的地图，简化版"""
    # 使用矢量街道地图，更清晰
    m = folium.Map(location=[center[1], center[0]], zoom_start=17, tiles=VEC_URL, attr=ATTR)
    ...
    # 添加完整航线
    if full_path and len(full_path) > 1:
        folium.PolyLine([[p[1], p[0]] for p in full_path], color='green', weight=3, opacity=0.8, 
                       popup="规划航线").add_to(m)
    ...
    if hist and len(hist) > 1:
        trail = [[p[1], p[0]] for p in hist[-50:] if len(p) == 2]
        if len(trail) > 1:
            folium.PolyLine(trail, color='orange', weight=2, opacity=0.7, popup="历史轨迹").add_to(m)
    
    # 添加无人机当前位置
    if drone_pos and drone_pos[0] != 0:
        folium.Marker([drone_pos[1], drone_pos[0]], 
                     popup=f"📍 无人机\n高度:{drone_alt}m",
                     icon=folium.Icon(color='red', icon='plane', prefix='fa')).add_to(m)
        
        # 添加安全半径圆圈
        folium.Circle([drone_pos[1], drone_pos[0]], radius=safe_rad,
                     color='blue', fill=True, fill_opacity=0.2, popup=f"安全区 {safe_rad}m").add_to(m)
...
def main():
    ...
    if page == "规划":
        ...
        # 自动刷新控制
        auto_refresh = st.checkbox("🔄 自动刷新 (0.3秒/次)", value=True, key="auto_refresh_monitor")
        ...
        # 确定地图中心
        if st.session_state.running and d.get('lat', 0) != 0:
            center = [d['lat'], d['lng']]
        ...
        # 生成监控地图
        drone_pos = [d['lng'], d['lat']] if d.get('lat', 0) != 0 else None
        m = make_monitor_map(
            center=center,
            waypoints=st.session_state.waypoints,
            obs=st.session_state.obs,
            hist=st.session_state.hist,
            full_path=st.session_state.full_path,
            drone_pos=drone_pos,
            drone_alt=d.get('altitude', 50),
            safe_rad=st.session_state.safe_rad
        )
        
        # 使用 st_folium 而不是 folium_static，确保地图正确渲染
        ...
        if st.session_state.running:
            if auto_refresh:
                st.info("🔄 自动刷新已开启，页面将每0.3秒自动更新")
            else:
                st.info("💡 提示：勾选「自动刷新」可实时查看飞行进度，或点击「手动刷新」手动更新")
        else:
