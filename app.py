...
def main():
    ...
    if page == "规划":
        ...
        with col1:
            start_btn = st.button("▶️ 开始/继续", use_container_width=True)
        with col2:
        ...
        if st.session_state.running:
            ...
            while st.session_state.running:
                ...
                # 获取最新数据
                d = st.session_state.hb.hist[0] if st.session_state.hb.hist else None
                if d is None:
                    break
            ...
            if d:
                ...
                col_a, col_b, col_c, col_d = st.columns(4)
                col_a.metric("🎯 当前航点", f"{current_wp_num}/{total_waypoints}" if total_waypoints > 0 else "0/0")
                col_b.metric("💨 飞行速度", "0 m/s")
                elapsed = d.get('elapsed', 0)
                col_c.metric("⏰ 已用时间", f"{int(elapsed//60):02d}:{int(elapsed%60):02d}")
                remaining = d.get('remaining_distance', 0)
                col_d.metric("📏 剩余距离", f"{remaining:.0f} m" if remaining >= 0 else "0 m")
                
                st.info(f"📍 最后位置: 经度 {d.get('lng', 0):.6f}, 纬度 {d.get('lat', 0):.6f}")
